...
local function check_replica_during_upgrade(replica)
    ...
    t.assert_equals(replica:eval("return tostring(err)"), "Can't modify data on a read-only instance - box.cfg.read_only is true")
    replica:eval("_, err = pcall(box.space.test.replace, box.space.test, {100, '100'})")
    t.assert_equals(replica:eval("return tostring(err)"), "Can't modify data on a read-only instance - box.cfg.read_only is true")
end
...
local function check_replica_after_upgrade(replica, is_ro)
    ...
    if is_ro then
        ...
        t.assert_equals(replica:eval("return tostring(err)"), "Tuple field 2 (y) type does not match one required by operation: expected string, got unsigned")
    end
...
g.test_space_upgrade = function(cg)
    ...
    cg.master:eval("box.schema.func.create('upgrade_func', {body = body, is_deterministic = true, is_sandboxed = true})")
    cg.master:eval("new_format = {{name='x', type='unsigned'}, {name='y', type='string'}}")
    cg.master:eval("f = box.space.test:upgrade({mode = 'notest', func = 'upgrade_func', format = new_format, background = true})")
